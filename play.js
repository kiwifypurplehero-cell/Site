...
import {PlaytimeTracker,descriptor} from './playtime-tracker.js';
...
const gameOrigin = playUrl ? new URL(playUrl).origin : null;
const playtime=new PlaytimeTracker(descriptor({source:'git',system:'web',id:game.repo||game.playUrl,title:game.name}));
playtime.start();
...
function loadGame() { playtime.markPaused(); releaseAll(); clearTimeout(loadTimer); document.querySelector('#loading').hidden = false; document.querySelector('#error').hidden = true; if (!playUrl) return showLoadError(); frame.src = 'about:blank'; requestAnimationFrame(() => { frame.src = playUrl; loadTimer = setTimeout(showLoadError, 20000); }); }
function showLoadError() { playtime.markPaused(); clearTimeout(loadTimer); document.querySelector('#loading').hidden = true; document.querySelector('#error').hidden = false; }
...
function presetButtons(name) {
  // Undertale's native keyboard controls: arrows move, Z confirms, X cancels and C opens the menu.
  if (name === 'Undertale') return [['ArrowUp', 18, 64], ['ArrowLeft', 10, 78], ['ArrowDown', 18, 78], ['ArrowRight', 26, 78], ['Z', 84, 76], ['X', 92, 66], ['C', 92, 86]].map(v => makeButton(...v));
...
function bridgeNote() { return `<p class="bridge-note ${bridgeAvailable ? 'connected' : ''}">Bridge: ${bridgeAvailable ? 'conectado' : 'não disponível'}</p>`; }
function renderLoadouts() { const entry = gameLoadouts(); const selected = current(entry); const undertalePreset = gameKey === 'undertale' ? '<button type="button" data-preset="Undertale">Undertale</button>' : ''; loadoutsContent.innerHTML = `<p class="loadout-summary">LDT atual: <strong>${escapeHtml(selected?.name || 'Nenhum')}</strong></p><div class="loadout-list">${entry.loadouts.map(item => `<button type="button" class="loadout-card ${item.id === entry.selected ? 'selected' : ''}" data-select-loadout="${item.id}"><strong>${escapeHtml(item.name)}</strong><small>${item.type === 'PS5' ? 'PS5' : 'Teclado'} · ${item.buttons.length} botões</small></button>`).join('') || '<p class="loadout-summary">Nenhum LDT salvo.</p>'}</div><label class="hud-toggle"><input type="checkbox" data-visible ${entry.visible ? 'checked' : ''}> Mostrar controles na tela</label><div class="loadout-actions"><button type="button" data-create="PC">Criar LDT</button><button type="button" data-create="PS5">Criar LDT PS5</button>${undertalePreset}<button type="button" data-preset="WASD + Space">WASD + Space</button><button type="button" data-preset="Setas + Enter">Setas + Enter</button><button type="button" data-preset="PS5 básico">PS5 básico</button>${selected ? '<button type="button" data-layout>Editar LDT</button><button type="button" data-delete-loadout>Excluir LDT</button>' : ''}</div>${bridgeNote()}`; }
...
function sendVirtualKey({ action, key, code }) {
  if (bridgeAvailable && gameOrigin) { frame.contentWindow?.postMessage({ type: 'plumpgames-input', action, key, code }, gameOrigin); return true; }
  return false;
}
function updateBridgeStatus(available) { bridgeAvailable = available; if (!document.querySelector('#loadouts-view').hidden) renderLoadouts(); }
function detectBridge() { clearTimeout(bridgeTimer); updateBridgeStatus(null); if (!gameOrigin) return updateBridgeStatus(false); frame.contentWindow?.postMessage({ type: 'plumpgames-input-ping' }, gameOrigin); bridgeTimer = setTimeout(() => { if (bridgeAvailable === null) updateBridgeStatus(false); }, 1500); }
