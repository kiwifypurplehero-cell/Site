...
async function safeInit(name,initializer){try{await initializer()}catch(error){console.error(`Falha ao iniciar ${name}:`,error)}}
async function main(){
  installDialogControls();
  await safeInit('autenticação',initAuth);
  await safeInit('perfil',initProfile);
  await safeInit('live wallpapers',initWallpapers);
...
main().catch(error=>console.error('Falha ao iniciar aplicação:',error));
