import {playerProfile,formatPlaytime} from './player-profile.js';
...
try{const response=await fetch('/api/player/games',{credentials:'same-origin',headers:{'X-PlumpGames-Player':playerProfile.get().guestPlayerId}});if(response.ok){const data=await response.json();played=new Map((data.games||[]).map(game=>[game.gameId,game]));decorate();}}catch{}
