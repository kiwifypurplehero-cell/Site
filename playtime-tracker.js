...
export class PlaytimeTracker extends EventTarget {
  ...
  presence(game=this.game){if(!globalThis.window?.plumpUser||window.plumpUser.isGuest)return;send('/api/presence/heartbeat',game?{currentGameId:game.gameId,currentGameTitle:game.title}:{currentGameId:null,currentGameTitle:null}).catch(()=>{});}
  markRunning(){if(this.running)return;this.running=true;this.everStarted=true;this.ensureSession().catch(()=>{});this.presence();clearInterval(this.presenceTimer);this.presenceTimer=setInterval(()=>this.presence(),90_000);this.refreshLock();this.resumeClock();this.timer=setInterval(()=>this.flush(),this.heartbeatMs);this.lockTimer=setInterval(()=>this.refreshLock(),LOCK_TTL/2);}
