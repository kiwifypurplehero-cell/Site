...
A aplicação exige uma conta antes de carregar o catálogo. O Worker usa o D1 existente para `users`, sessões e preferências. Senhas são derivadas com Web Crypto PBKDF2-HMAC-SHA-256 (100.000 iterações e salt aleatório individual) e persistidas no formato versionado `pbkdf2-sha256$100000$SALT_BASE64$HASH_BASE64`; o formato anterior `pbkdf2$sha256$...` continua aceito na verificação. A verificação lê as iterações do hash, aceita no mínimo 100.000 e, por segurança no runtime atual, recusa hashes acima de 100.000 sem executar a derivação. Contas antigas com hashes de 210.000 iterações, caso existam, precisam usar a recuperação de senha em vez de provocar uma falha interna no Worker. Sessões duram 30 dias e usam cookie `HttpOnly`, `Secure`, `SameSite=Lax`.
